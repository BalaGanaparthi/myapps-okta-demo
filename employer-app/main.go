...
var (
	...
	sessionStore = sessions.NewCookieStore([]byte("okta-hosted-login-session-store-employer"))
	state        = generateState()
...
func main() {
	...
	log.Print("server starting at localhost:8082 ... ")
...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	q.Add("scope", "openid profile email employer")
	q.Add("redirect_uri", "http://localhost:8082/authorization-code/callback")
	q.Add("state", state)
	q.Add("nonce", nonce)
...
func AuthCodeCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	session, err := sessionStore.Get(r, "okta-hosted-login-session-store-employer")
	if err != nil {

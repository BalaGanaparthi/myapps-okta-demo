...
var (
	...
	sessionStore = sessions.NewCookieStore([]byte("okta-hosted-login-session-store-nonMember"))
	state        = generateState()
...
func main() {
	...
	log.Print("server starting at localhost:8085 ... ")
...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	q.Add("scope", "openid profile email non_member")
	q.Add("redirect_uri", "http://localhost:8085/authorization-code/callback")
	q.Add("state", state)
	q.Add("nonce", nonce)
...
func AuthCodeCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	session, err := sessionStore.Get(r, "okta-hosted-login-session-store-nonMember")
	if err != nil {

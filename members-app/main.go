...
var (
	...
	sessionStore = sessions.NewCookieStore([]byte("okta-hosted-login-session-store-member"))
	state        = generateState()
...
func main() {
	...
	log.Print("server starting at localhost:8084 ... ")
	err := http.ListenAndServe("localhost:8084", nil)
	if err != nil {
...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
	...
	q.Add("scope", "openid profile email member")
	q.Add("redirect_uri", "http://localhost:8084/authorization-code/callback")
	q.Add("state", state)
	q.Add("nonce", nonce)
...
func AuthCodeCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	session, err := sessionStore.Get(r, "okta-hosted-login-session-store-member")
	if err != nil {

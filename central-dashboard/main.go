...
var (
	...
	sessionStoreAT  = sessions.NewCookieStore([]byte("okta-hosted-login-session-store-AT"))
	sessionStoreIdT = sessions.NewCookieStore([]byte("okta-hosted-login-session-store-IdT"))
	state           = generateState()
...
func HomeHandler(w http.ResponseWriter, r *http.Request) {
	...
	// fmt.Print("Populated Profile and IsAuthenticated\n")
...
func AuthCodeCallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	sessionAT, err := sessionStore.Get(r, "okta-hosted-login-session-storeAT")

	sessionIdT, err := sessionStore.Get(r, "okta-hosted-login-session-storeIdT")
	...
	if verificationError == nil {
		...
		sessionIdT.Values["id_token"] = exchange.IdToken
		sessionAT.Values["access_token"] = exchange.AccessToken

		session.Save(r, w)
		sessionAT.Save(r, w)
		sessionIdT.Save(r, w)
		fmt.Print("IdToken is saved to Sesison\n")
	}
...
func AccessTokenHandler(w http.ResponseWriter, r *http.Request) {
	...
	session, err := sessionStore.Get(r, "okta-hosted-login-session-storeAT")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
...
func IDTokenHandler(w http.ResponseWriter, r *http.Request) {
	...
	session, err := sessionStore.Get(r, "okta-hosted-login-session-storeIdT")
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
...
func LogoutHandler(w http.ResponseWriter, r *http.Request) {
	...
	// - Start of D1 Gateway code

	sessionAT, err := sessionStore.Get(r, "okta-hosted-login-session-storeAT")
	sessionIdT, err := sessionStore.Get(r, "okta-hosted-login-session-storeIdT")

	idToken := sessionIdT.Values["id_token"]

	delete(session.Values, "isAuthenticated")
	delete(sessionAT.Values, "access_token")
	delete(sessionIdT.Values, "id_token")
...
func isAuthenticated(r *http.Request) bool {
	...
	if err != nil || session.Values["isAuthenticated"] == nil || session.Values["isAuthenticated"] == "" {
		return false
	}
